    ...
    def spammer(token, message):
        try:
            response = requests.post(
                ...
                headers={
                    'User-Agent': 'Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US; rv:1.7.12) Gecko/20950915 Firefox/1.0.7',
                    'Authorization': token
                }
            )
            ...
            print(f"{BEFORE + current_time_hour() + AFTER} {GEN_INVALID} Message: {white}{message_sensur}{red} Channel: {white}{channel}{red} Status: {white}Error: {str(e)}{red}")
