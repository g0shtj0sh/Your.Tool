...
try:
    ...
    def CheckPassword(password_test):
        ...
        try:
            ...
            # Vérifier que le mot de passe chiffré n'est pas vide
            ...
            methods = {
                ...
                '5': lambda pwd: pbkdf2_hmac('sha256', pwd.encode('utf-8'), salt if salt else b'default_salt', 100000).hex() == encrypted_password,
                '6': lambda pwd: base64.b64decode(encrypted_password.encode('utf-8')).decode('utf-8') == pwd
            }
            
            # Vérifier que la méthode est supportée
            if choice not in methods:
                print(f"{BEFORE + current_time_hour() + AFTER} {ERROR} Unsupported encryption method: {choice}")
                return False
            
            return methods[choice](password_test)
        except Exception as e:
