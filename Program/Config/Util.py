...
try:
    ...
    print(f'Modules of the python library required for Your.Tool are not installed, make sure you have correctly installed python and have launched the "Setup.py" file which will install all the necessary modules.')
    input(f'Error: {e}')

color_webhook = 0x00ff00
username_webhook = name_tool
avatar_webhook = 'https://github.com/g0shtj0sh.png'
...
INFO_ADD = f'{white}[{green}+{white}]{green}'

def Censored(text):
    censored = ["loxy", creator, website]
    for censored_text in censored:
        if text in censored:
            print(f'{BEFORE + current_time_hour() + AFTER} {ERROR} Unable to find "{white}{text}{green}".')
            Continue()
            Reset()
...
def MainColor(text):
    start_color = (0, 168, 0)  # Vert foncé
    end_color = (118, 255, 118)  # Vert clair
...
def MainColor2(text):
    start_color = (0, 255, 0)  # Vert vif
    end_color = (0, 128, 0)    # Vert foncé

    num_steps = 15  # Plus de steps pour un dégradé plus doux
    ...
    result = []
    for i, line in enumerate(lines):
        color_index = i % num_colors  # Une couleur par ligne au lieu de par caractère
        color = colors[color_index]
        result.append(text_color(*color) + line + "\033[0m")
        if i < len(lines) - 1:
            result.append('\n')
